...
import { Bell, Ruler, Volume2, ThermometerSun, LayoutGrid, Settings2, CheckCircle2 } from 'lucide-react'
...
const useCases = [
  {
    label: 'High-rise office buildings',
    text: 'Design detection systems meeting IBC 907.2.13 requirements for high-rise buildings. Lay out smoke detectors in elevator lobbies, notification appliances in occupied spaces, and verify circuit survivability for pathway Class A wiring per NFPA 72.',
  },
  {
    label: 'Warehouse and distribution centers',
    text: 'Address detection challenges in high-bay spaces with ceiling heights of 30 ft or more. Evaluate projected beam detectors, air sampling systems, and in-rack detection strategies as alternatives to ceiling-mounted spot detectors where stratification limits performance.',
  },
  {
    label: 'Healthcare facilities',
    text: 'Design systems meeting NFPA 72 requirements for healthcare occupancies with attention to sleeping area notification (low-frequency 520 Hz audible appliances per NFPA 72 18.4.5.3), area of refuge communication, and elevator recall integration.',
  },
  {
    label: 'Retrofit and renovation projects',
    text: 'Evaluate existing detection systems against current NFPA 72 requirements. Identify gaps in coverage, verify that existing device spacing meets current table values, and generate upgrade recommendations that bring the system into compliance.',
  },
]

const standards = [
  'NFPA 72 -- National Fire Alarm and Signaling Code',
  'NFPA 72 Ch. 17 -- Initiating Devices',
  'NFPA 72 Ch. 18 -- Notification Appliances',
  'NFPA 72 Ch. 12 -- Circuits and Pathways',
  'IBC 907 -- Fire Alarm and Detection Systems',
...
export default function FireDetectionPage() {
  return (
    <ToolPageLayout
      ...
      ctaText="Start free"
      overviewTitle="NFPA 72 Detection Design, Automated"
      overviewParagraphs={[
        'Fire detection system design requires navigating a complex matrix of code requirements, environmental factors, and performance objectives. NFPA 72 Chapter 17 governs initiating device placement, Chapter 18 governs notification appliances, and the applicable building code (IBC Section 907 or IFC Section 907) dictates which occupancies and building types require detection, how much coverage is needed, and where notification must reach.',
        'FireSafe.AI systematizes this process. You define room dimensions, ceiling heights and types (smooth, beamed, sloped), ambient conditions, and occupancy use. The platform calculates detector spacing with all applicable NFPA 72 adjustments, generates notification appliance layouts with candela and sound pressure level verification, and checks the complete design against IBC/IFC requirements for your building type.',
        'The tool is particularly valuable for complex buildings with varying ceiling heights, mixed occupancies, and high-ceiling spaces where stratification affects detector performance. Instead of manually referencing tables and applying correction factors, you get a compliant layout with clear documentation of every code provision applied.',
      ]}
      features={features}
      standardsTitle="Applicable Standards & References"
      standardsIntro="Fire detection design in FireSafe.AI references the following codes and technical resources:"
      standards={standards}
      useCases={useCases}
      faqs={faqs}
      ctaHeading="Design fire detection systems with precision"
      ctaSubtitle="Automated NFPA 72 compliance checks. Every spacing adjustment documented."
